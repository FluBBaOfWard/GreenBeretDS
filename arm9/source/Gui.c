...
#define EMUVERSION "V0.6.2 2024-03-02"
...
void uiAbout() {
	...
	drawMenuText("Up:     Jump/Climb",7,0);
	drawMenuText("Down:   Crouch",8,0);
	drawMenuText("B:      Knife attack",9,0);
	drawMenuText("A:      Special attack",10,0);

	drawMenuText("GreenBeretDS " EMUVERSION, 20, 0);
	drawMenuText("ARMZ80       " ARMZ80VERSION, 21, 0);
	drawMenuText("K005849      " K005849VERSION, 22, 0);
...
//	char s[10];
	...
	drawSubItem("Bonus:", bonusTxt[(gDipSwitch1>>3)&3]);
...
void uiLoadGame() {
	...
	int i;
	for (i=0; i<ARRSIZE(gberetGames); i++) {
		drawSubItem(gberetGames[i].fullName, NULL);
	}
}
